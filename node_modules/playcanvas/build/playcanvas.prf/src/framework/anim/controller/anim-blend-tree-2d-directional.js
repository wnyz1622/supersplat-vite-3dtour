	...
	calculateWeights() {
		...
		for (let i = 0; i < this._children.length; i++) {
			...
			for (let j = 0; j < this._children.length; j++) {
				...
				AnimBlendTreeDirectional2D._pip.set((pLength - piLength) / ((pjLength + piLength) / 2), Vec2.angleRad(pi, AnimBlendTreeDirectional2D._p) * 2);
				const result = math.clamp(1 - Math.abs(AnimBlendTreeDirectional2D._pip.dot(pipj) / pipj.lengthSq()), 0, 1);
